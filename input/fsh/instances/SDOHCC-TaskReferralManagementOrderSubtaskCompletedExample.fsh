...
Description: "An example of a completed Task For Referral Management that delegates responsibility to fulfill a referral from an intermediary to another organization (e.g., when an intermediary's coordination platform (CP) receives a Task from a referral source and passes it on to a Community Based Organization)."
Usage: #example
