...
* extension[pertainsToGoal] ^short = "Pertains to SDOH goal"
  ...
  * ^min = 0
  * value[x] only Reference(SDOHCCGoal)
  * value[x] MS
...
* code from USCoreProcedureCodes (required)
  ...
  * ^definition = "A code that identifies a particular service (e.g., procedure) that has been requested."
  * ^comment = "As the Gravity Project continues to refine content for the SDOH categories (e.g., food, housing, transportation, etc.), this value set will be refined to codes that pertain to SDOH categories. For further guidance on external value sets vetted by the Gravity Project see: [SDOH terminology guidance](support_for_multiple_domains.html#external-value-set-guidance)"
