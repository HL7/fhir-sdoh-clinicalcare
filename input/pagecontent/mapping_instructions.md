In this IG, information is first captured in QuestionnaireResponses and is then automatically transformed into Observations and Conditions (where appropriate) using the HL7 StructureMap resource. This approach is selected because it supports complex transformation of data and allows the conversion process between data and Questionnaire to be maintained independently. The StuctureMap-based extraction mechanism (considerations, error handling and other details) is documented in the HL7 Structured Data Capture IG [here](https://build.fhir.org/ig/HL7/sdc/extraction.html#structuremap-based-extraction). Examples are provided of a Questionnaire, QuestionnaireResponse, resulting Observations and Conditions, and the StructureMap used to convert from one to the other:
...
The StructureMap resource examples in this IG are generated using the FHIR Mapping Language. Instructions and tutorials on how to use the FHIR Mapping Language to convert and transform resources are found [here](https://confluence.hl7.org/display/FHIR/Using+the+FHIR+Mapping+Language).

The following are the contents of the FHIR Mapping Language file that can be used to generate the StructureMaps for the Hunger Vital Sign (HVS) and PRAPARE Questionnaires:

1. [Hunger Vital Sign (HVS) FHIR Mapping Language file](mapping_instructions.html#hvs)
2. [PRAPARE FHIR Mapping Language file](mapping_instructions.html#prapare)
